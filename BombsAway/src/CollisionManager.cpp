		...
		if (!object2->getIsColliding())
		{
			...
			case WALL:
				...
				if (TheGame::Instance()->checkForKeystroke(SDL_SCANCODE_W))
				{
					...
					if (object1->getPosition().y <= 0) // Top of screen
					{
						object1->setPosition(glm::vec2(object1->getPosition().x, 0));
					}
				}
				if (TheGame::Instance()->checkForKeystroke(SDL_SCANCODE_S))
				{
					...
					if (object1->getPosition().y >= (768 - object1->getHeight())) // Bottom of screen minus player height
					{
						object1->setPosition(glm::vec2(object1->getPosition().x, (768 - object1->getHeight())));
					}
				}
...
bool CollisionManager::tileCollisionCheck(GameObject* object, int tileMap[12][20])
{
	...
	int objTop = object->getPosition().y / 64;
	int objBottom = (object->getPosition().y + object->getHeight() - 1) / 64;
	...
	if (objBottom > WINDOW_HEIGHT/64)
	{
		objBottom = WINDOW_HEIGHT/64;
	}
...
bool CollisionManager::playerCollisionMovement(GameObject* object, int tileMap[12][20])
{
	...
	if (TheGame::Instance()->checkForKeystroke(SDL_SCANCODE_W))
	{
		...
		if (object->getPosition().y <= 0) // Top of screen
		{
			object->setPosition(glm::vec2(object->getPosition().x, 0));
		}
	}
	if (TheGame::Instance()->checkForKeystroke(SDL_SCANCODE_S))
	{
		...
		if (object->getPosition().y >= (768 - object->getHeight())) // Bottom of screen
		{
			object->setPosition(glm::vec2(object->getPosition().x, (768 - object->getHeight())));
		}
	}
